...
- refreshed `.github/skills/code-review/SKILL.md` to replace the matchable hyphenated Slack `xoxb` token prefix in the secret-hygiene bullet with inert vendor-prefix wording
- refreshed `CLAUDE.md` and `.github/copilot-instructions.md` to document the `checks.yaml` rebase and changelog-fragment gate that runs on every pull request
...
- refreshed `CLAUDE.md` and `.github/copilot-instructions.md` to document the `claude-review.yaml` and `claude-mention.yaml` workflows and the `.github/skills/code-review/` skill added since these files were last updated
